...
const commentValidation = require("./validate");
...
const createObject = async (req, res) => {
	try {
		...
		const object = new Comment(req.body);
		await object.save();
...
const getObjects = async (req, res) => {
	try {
		const objects = await Comment.find(req.query);
		res.json(objects);
	} catch (err) {
		res.status(500).json(err.message);
	}
...
const checkObjectExists = async (req, res, next) => {
	try {
		...
		const object = await Comment.findById(id);
		if (!object) {
...
const updateObjectById = async (req, res) => {
	try {
		const { _id: id } = req.object;
		const object = await Comment.findByIdAndUpdate(id, req.body, {
			new: true,
		});
		res.json(object);
	} catch (err) {
